...
Flux.@functor QNNL_faithful

function apply(ql::QNNL_faithful{T}, x) where T
    cir = chain(ql.encoder, ql.transform)
    n = nqubits(cir)
    dispatch!(cir, [x; ql.params])
    ...
    y = T.([real(sum(measure(ql.Hami[i], psi; nshots = ql.nshots))) / ql.nshots for i = 1:m])
    if size(ql.bias, 1) > 0
        y += ql.bias
    end
    return y
end
(ql::QNNL_faithful{T})(x) where {T} = apply(ql, x)

params(ql::QNNL_faithful{T}) where {T} = params(ql.params, ql.bias)

function Zygote._pullback(cx::AContext, ql::QNNL_faithful{T}, x) where {T}
    out = apply(ql, x)
    out, function pullback(h̄)
        ∇ql, ∇x = faithfulAD(ql, x, h̄)
        ∇W = ∇ql[1]
        ∇b = ∇ql[2]
        Zygote.accum_param(cx, ql.params, ∇W)
        Zygote.accum_param(cx, ql.bias, ∇b)
...
function faithfulAD(ql::QNNL_faithful{T}, x, h̄) where {T}
    m = size(ql.Hami, 1)
    s1 = size(x, 1)
    s2 = size(ql.params, 1)
    s3 = size(ql.bias, 1)
